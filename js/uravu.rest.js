    ...
    Resource = (function() {
        ...
        Resource.prototype.ajax = function(method, url, data) {
            ...
            if (this.opts.apiKey) {
                ...
                headers.Auth = this.opts.apiKey;
            }
            
            if (data && this.opts.stringifyData && (method !== 'GET' && method !== 'HEAD')) {
                data = stringify(data);
                headers['Content-Type'] = "application/json"; // this line creates CORS issue
                //headers['Content-Type'] = "text/plain"; // its for test but works event without this (Jana)
            }
            ...
            req.always(function(rs) {
                if (typeof(rs.data) !== "undefined" && rs.data === "invalid session, you must login first") {
                    var page = $(':mobile-pagecontainer').pagecontainer('getActivePage')[0].id;
                    console.log('your session have been expired');
                    $.removeCookie('charing_uid');
                    $.removeCookie('charing_email');
                    $.removeCookie('charing_session');
                    if(page !== 'login' ) {
